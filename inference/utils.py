...
def visualize(image: torch.Tensor, n_mask: int=20, num_images=8, is_mask=False):
    ...
    n_per_row = min(num_images, h)
    # to avoid the number of rows and cols is too unbalanced (too many rows)
    # image: b 1 w d
    n_per_col = image.shape[0] // n_per_row
    tolerance = 8
    if n_per_col / n_per_row > tolerance:
        n_per_row = round(math.sqrt(image.shape[0]))
    image = make_grid(image, nrow=n_per_row, normalize=not is_mask, pad_value=n_mask if is_mask else 1, padding=3)
