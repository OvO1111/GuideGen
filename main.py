import argparse, os, sys, datetime, glob, re, torchio
import numpy as np
import time
import torch
import wandb
...
class DataModuleFromConfig(pl.LightningDataModule):
    def __init__(self, 
                 ...
                 train=None, validation=None, test=None, common=None,
                 wrap=False, 
                 num_workers=None, 
        ...
        if train is not None:
            self.dataset_configs["train"] = OmegaConf.merge(common, train)
            self.train_dataloader = self._train_dataloader
        if validation is not None:
            self.dataset_configs["validation"] = OmegaConf.merge(common, validation)
            self.val_dataloader = partial(self._val_dataloader, shuffle=shuffle_val_dataloader)
        if test is not None:
            self.dataset_configs["test"] = OmegaConf.merge(common, test)
...
class ImageLogger(Callback):
    def __init__(self, 
        ...
        self.log_steps_tr = [] if self.batch_freq_tr != 0 else []
        self.log_steps_val = [] if self.batch_freq_val != 0 else []
        self.log_steps_te = [] if self.batch_freq_te != 0 else []
    ...
    def log_local(self, save_dir, split, images, global_step, current_epoch, batch_idx):
        ...
        filename = "gs-{:06}_e-{:06}_b-{:06}.svg".format(
                global_step,
                current_epoch,
                batch_idx)
