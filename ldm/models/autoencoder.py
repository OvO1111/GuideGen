...
class VQModel(pl.LightningModule):
    def __init__(self,
                 ...
                 use_window_norm_in_forward=False,
                 ):
        super().__init__()
        ...
        self.use_window_norm = use_window_norm_in_forward
    ...
    def _process_input(self, x: torch.Tensor, y: torch.Tensor):
...
class AutoencoderKL(pl.LightningModule):
    ...
    def encode_to_latents(self, x, c_cat=None):
        h = self.encoder(x, {f"c_{self.conditioning_key}": c_cat} if c_cat is not None else None)
        moments = self.quant_conv(h)
        return moments
    
    def decode_from_latents(self, moments, c_cat=None, sample_posterior=True):
        posterior = DiagonalGaussianDistribution(moments)
        if sample_posterior:
            z = posterior.sample()
        else:
            z = posterior.mode()
        dec = self.decode(z, c_cat)
        return dec
    ...
    def training_step(self, batch, batch_idx, optimizer_idx):
        ...
        if optimizer_idx == 0:
            aeloss, log_dict_ae = self.loss(
                *self._process_input(inputs, reconstructions), 
                posterior=posterior, 
